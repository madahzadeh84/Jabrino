...
/**
 * پرانتز بیرونی را فقط وقتی حذف می کند که کل عبارت را پوشش داده باشد.
 */
...
function detectConjugate(expr) {
  ...
  const sameLeft = sameText(a1.left, a2.left);
  const sameRight = sameText(a1.right, a2.right);

  const oppositeSigns =
    (a1.sign === "+" && a2.sign === "-") ||
    (a1.sign === "-" && a2.sign === "+");

  if (sameLeft && sameRight && oppositeSigns) {
    return {
      kind: "CONJUGATE",
      raw: s,
      normalized: s,
